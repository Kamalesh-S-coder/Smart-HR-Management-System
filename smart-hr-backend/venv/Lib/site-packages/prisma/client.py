# -*- coding: utf-8 -*-
# code generated by Prisma. DO NOT EDIT.
# pyright: reportUnusedImport=false
# fmt: off
from __future__ import annotations

# global imports for type checking
from builtins import bool as _bool
from builtins import int as _int
from builtins import float as _float
from builtins import str as _str
import sys
import decimal
import datetime
from typing import (
    TYPE_CHECKING,
    Optional,
    Iterable,
    Iterator,
    Sequence,
    Callable,
    ClassVar,
    NoReturn,
    TypeVar,
    Generic,
    Mapping,
    Tuple,
    Union,
    List,
    Dict,
    Type,
    Any,
    Set,
    overload,
    cast,
)
from typing_extensions import TypedDict, Literal


LiteralString = str
# -- template client.py.jinja --
import warnings
import logging
from datetime import timedelta
from pathlib import Path
from types import TracebackType
from typing_extensions import override

from pydantic import BaseModel

from . import types, models, errors, actions
from ._base_client import BasePrisma, UseClientDefault, USE_CLIENT_DEFAULT
from .types import DatasourceOverride, HttpConfig, MetricsFormat
from ._types import BaseModelT, PrismaMethod, TransactionId, Datasource
from .bases import _PrismaModel
from ._builder import QueryBuilder, dumps
from .generator.models import EngineType, OptionalValueFromEnvVar, BinaryPaths
from ._compat import removeprefix, model_parse
from ._constants import DEFAULT_CONNECT_TIMEOUT, DEFAULT_TX_MAX_WAIT, DEFAULT_TX_TIMEOUT
from ._raw_query import deserialize_raw_results
from ._metrics import Metrics
from .metadata import PRISMA_MODELS, RELATIONAL_FIELD_MAPPINGS
from ._transactions import AsyncTransactionManager, SyncTransactionManager

# re-exports
from ._base_client import SyncBasePrisma, AsyncBasePrisma, load_env as load_env
from ._registry import (
    register as register,
    get_client as get_client,
    RegisteredClient as RegisteredClient,
)


__all__ = (
    'ENGINE_TYPE',
    'SCHEMA_PATH',
    'BINARY_PATHS',
    'Batch',
    'Prisma',
    'Client',
    'load_env',
    'register',
    'get_client',
)

log: logging.Logger = logging.getLogger(__name__)

SCHEMA_PATH = Path('C:/Users/User/smart-hr-backend/prisma/schema.prisma')
PACKAGED_SCHEMA_PATH = Path(__file__).parent.joinpath('schema.prisma')
ENGINE_TYPE: EngineType = EngineType.binary
BINARY_PATHS = model_parse(BinaryPaths, {'queryEngine': {'windows': 'C:\\Users\\User\\AppData\\Local\\npm-cache\\_npx\\0442896622ed0e65\\node_modules\\prisma\\query-engine-windows.exe'}, 'introspectionEngine': {}, 'migrationEngine': {}, 'libqueryEngine': {}, 'prismaFmt': {}})


class Prisma(AsyncBasePrisma):
    # Note: these property names can be customised using `/// @Python(instance_name: '...')`
    # https://prisma-client-py.readthedocs.io/en/stable/reference/schema-extensions/#instance_name
    role: 'actions.RoleActions[models.Role]'
    user: 'actions.UserActions[models.User]'
    devicefingerprint: 'actions.DeviceFingerprintActions[models.DeviceFingerprint]'
    auditlog: 'actions.AuditLogActions[models.AuditLog]'
    employeeprofile: 'actions.EmployeeProfileActions[models.EmployeeProfile]'
    attendancerecord: 'actions.AttendanceRecordActions[models.AttendanceRecord]'
    payrollrecord: 'actions.PayrollRecordActions[models.PayrollRecord]'
    leaverequest: 'actions.LeaveRequestActions[models.LeaveRequest]'
    shift: 'actions.ShiftActions[models.Shift]'

    __slots__ = (
        'role',
        'user',
        'devicefingerprint',
        'auditlog',
        'employeeprofile',
        'attendancerecord',
        'payrollrecord',
        'leaverequest',
        'shift',
    )

    def __init__(
        self,
        *,
        use_dotenv: bool = True,
        log_queries: bool = False,
        auto_register: bool = False,
        datasource: DatasourceOverride | None = None,
        connect_timeout: int | timedelta = DEFAULT_CONNECT_TIMEOUT,
        http: HttpConfig | None = None,
    ) -> None:
        super().__init__(
            http=http,
            use_dotenv=use_dotenv,
            log_queries=log_queries,
            datasource=datasource,
            connect_timeout=connect_timeout,
        )
        self._set_generated_properties(
            schema_path=SCHEMA_PATH,
            engine_type=ENGINE_TYPE,
            prisma_models=PRISMA_MODELS,
            packaged_schema_path=PACKAGED_SCHEMA_PATH,
            relational_field_mappings=RELATIONAL_FIELD_MAPPINGS,
            active_provider='sqlite',
            default_datasource_name='db',
        )

        self.role = actions.RoleActions[models.Role](self, models.Role)
        self.user = actions.UserActions[models.User](self, models.User)
        self.devicefingerprint = actions.DeviceFingerprintActions[models.DeviceFingerprint](self, models.DeviceFingerprint)
        self.auditlog = actions.AuditLogActions[models.AuditLog](self, models.AuditLog)
        self.employeeprofile = actions.EmployeeProfileActions[models.EmployeeProfile](self, models.EmployeeProfile)
        self.attendancerecord = actions.AttendanceRecordActions[models.AttendanceRecord](self, models.AttendanceRecord)
        self.payrollrecord = actions.PayrollRecordActions[models.PayrollRecord](self, models.PayrollRecord)
        self.leaverequest = actions.LeaveRequestActions[models.LeaveRequest](self, models.LeaveRequest)
        self.shift = actions.ShiftActions[models.Shift](self, models.Shift)

        if auto_register:
            register(self)

    @property
    @override
    def _default_datasource(self) -> Datasource:
        return {
            'name': 'db',
            'url': OptionalValueFromEnvVar(**{'value': None, 'fromEnvVar': 'DATABASE_URL'}).resolve(),
        }

    async def execute_raw(self, query: LiteralString, *args: Any) -> int:
        resp = await self._execute(
            method='execute_raw',
            arguments={
                'query': query,
                'parameters': args,
            },
            model=None,
        )
        return int(resp['data']['result'])

    @overload
    async def query_first(
        self,
        query: LiteralString,
        *args: Any,
    ) -> dict[str, Any]:
        ...

    @overload
    async def query_first(
        self,
        query: LiteralString,
        *args: Any,
        model: Type[BaseModelT],
    ) -> Optional[BaseModelT]:
        ...

    async def query_first(
        self,
        query: LiteralString,
        *args: Any,
        model: Optional[Type[BaseModelT]] = None,
    ) -> Union[Optional[BaseModelT], dict[str, Any]]:
        """This function is the exact same as `query_raw()` but returns the first result.

        If model is given, the returned record is converted to the pydantic model first,
        otherwise a raw dictionary will be returned.
        """
        results: Sequence[Union[BaseModelT, dict[str, Any]]]
        if model is not None:
            results = await self.query_raw(query, *args, model=model)
        else:
            results = await self.query_raw(query, *args)

        if not results:
            return None

        return results[0]

    @overload
    async def query_raw(
        self,
        query: LiteralString,
        *args: Any,
    ) -> List[dict[str, Any]]:
        ...

    @overload
    async def query_raw(
        self,
        query: LiteralString,
        *args: Any,
        model: Type[BaseModelT],
    ) -> List[BaseModelT]:
        ...

    async def query_raw(
        self,
        query: LiteralString,
        *args: Any,
        model: Optional[Type[BaseModelT]] = None,
    ) -> Union[List[BaseModelT], List[dict[str, Any]]]:
        """Execute a raw SQL query against the database.

        If model is given, each returned record is converted to the pydantic model first,
        otherwise results will be raw dictionaries.
        """
        resp = await self._execute(
            method='query_raw',
            arguments={
                'query': query,
                'parameters': args,
            },
            model=model,
        )
        result = resp['data']['result']
        if model is not None:
            return deserialize_raw_results(result, model=model)

        return deserialize_raw_results(result)

    def batch_(self) -> Batch:
        """Returns a context manager for grouping write queries into a single transaction."""
        return Batch(client=self)

    def tx(
        self,
        *,
        max_wait: Union[int, timedelta] = DEFAULT_TX_MAX_WAIT,
        timeout: Union[int, timedelta] = DEFAULT_TX_TIMEOUT,
    ) -> TransactionManager:
        """Returns a context manager for executing queries within a database transaction.

        Entering the context manager returns a new Prisma instance wrapping all
        actions within a transaction, queries will be isolated to the Prisma instance and
        will not be commited to the database until the context manager exits.

        By default, Prisma will wait a maximum of 2 seconds to acquire a transaction from the database. You can modify this
        default with the `max_wait` argument which accepts a value in milliseconds or `datetime.timedelta`.

        By default, Prisma will cancel and rollback ay transactions that last longer than 5 seconds. You can modify this timeout
        with the `timeout` argument which accepts a value in milliseconds or `datetime.timedelta`.

        Example usage:

        ```py
        async with client.tx() as transaction:
            user1 = await client.user.create({'name': 'Robert'})
            user2 = await client.user.create({'name': 'Tegan'})
        ```

        In the above example, if the first database call succeeds but the second does not then neither of the records will be created.
        """
        return TransactionManager(
            client=self,
            max_wait=max_wait,
            timeout=timeout,
        )


TransactionManager = AsyncTransactionManager[Prisma]


# TODO: this should return the results as well
# TODO: don't require copy-pasting arguments between actions and batch actions
class Batch:
    role: 'RoleBatchActions'
    user: 'UserBatchActions'
    devicefingerprint: 'DeviceFingerprintBatchActions'
    auditlog: 'AuditLogBatchActions'
    employeeprofile: 'EmployeeProfileBatchActions'
    attendancerecord: 'AttendanceRecordBatchActions'
    payrollrecord: 'PayrollRecordBatchActions'
    leaverequest: 'LeaveRequestBatchActions'
    shift: 'ShiftBatchActions'

    def __init__(self, client: Prisma) -> None:
        self.__client = client
        self.__queries: List[str] = []
        self._active_provider = client._active_provider
        self.role = RoleBatchActions(self)
        self.user = UserBatchActions(self)
        self.devicefingerprint = DeviceFingerprintBatchActions(self)
        self.auditlog = AuditLogBatchActions(self)
        self.employeeprofile = EmployeeProfileBatchActions(self)
        self.attendancerecord = AttendanceRecordBatchActions(self)
        self.payrollrecord = PayrollRecordBatchActions(self)
        self.leaverequest = LeaveRequestBatchActions(self)
        self.shift = ShiftBatchActions(self)

    def _add(self, **kwargs: Any) -> None:
        builder = QueryBuilder(
            **kwargs,
            prisma_models=PRISMA_MODELS,
            relational_field_mappings=RELATIONAL_FIELD_MAPPINGS,
        )
        self.__queries.append(builder.build_query())

    async def commit(self) -> None:
        """Execute the queries"""
        # TODO: normalise this, we should still call client._execute
        queries = self.__queries
        self.__queries = []

        payload = {
            'batch': [
                {
                    'query': query,
                    'variables': {},
                }
                for query in queries
            ],
            'transaction': True,
        }
        await self.__client._engine.query(
            dumps(payload),
            tx_id=self.__client._tx_id,
        )

    def execute_raw(self, query: LiteralString, *args: Any) -> None:
        self._add(
            method='execute_raw',
            arguments={
                'query': query,
                'parameters': args,
            }
        )

    async def __aenter__(self) -> 'Batch':
        return self

    async def __aexit__(
        self,
        exc_type: Optional[Type[BaseException]],
        exc: Optional[BaseException],
        exc_tb: Optional[TracebackType],
    ) -> None:
        if exc is None:
            await self.commit()


# NOTE: some arguments are meaningless in this context but are included
# for completeness sake
class RoleBatchActions:
    def __init__(self, batcher: Batch) -> None:
        self._batcher = batcher

    def create(
        self,
        data: types.RoleCreateInput,
        include: Optional[types.RoleInclude] = None
    ) -> None:
        self._batcher._add(
            method='create',
            model=models.Role,
            arguments={
                'data': data,
                'include': include,
            },
        )

    def create_many(
        self,
        data: List[types.RoleCreateWithoutRelationsInput],
        *,
        skip_duplicates: Optional[bool] = None,
    ) -> None:
        if self._batcher._active_provider == 'sqlite':
            raise errors.UnsupportedDatabaseError('sqlite', 'create_many()')

        self._batcher._add(
            method='create_many',
            model=models.Role,
            arguments={
                'data': data,
                'skipDuplicates': skip_duplicates,
            },
            root_selection=['count'],
        )

    def delete(
        self,
        where: types.RoleWhereUniqueInput,
        include: Optional[types.RoleInclude] = None,
    ) -> None:
        self._batcher._add(
            method='delete',
            model=models.Role,
            arguments={
                'where': where,
                'include': include,
            },
        )

    def update(
        self,
        data: types.RoleUpdateInput,
        where: types.RoleWhereUniqueInput,
        include: Optional[types.RoleInclude] = None
    ) -> None:
        self._batcher._add(
            method='update',
            model=models.Role,
            arguments={
                'data': data,
                'where': where,
                'include': include,
            },
        )

    def upsert(
        self,
        where: types.RoleWhereUniqueInput,
        data: types.RoleUpsertInput,
        include: Optional[types.RoleInclude] = None,
    ) -> None:
        self._batcher._add(
            method='upsert',
            model=models.Role,
            arguments={
                'where': where,
                'include': include,
                'create': data.get('create'),
                'update': data.get('update'),
            },
        )

    def update_many(
        self,
        data: types.RoleUpdateManyMutationInput,
        where: types.RoleWhereInput,
    ) -> None:
        self._batcher._add(
            method='update_many',
            model=models.Role,
            arguments={'data': data, 'where': where,},
            root_selection=['count'],
        )

    def delete_many(
        self,
        where: Optional[types.RoleWhereInput] = None,
    ) -> None:
        self._batcher._add(
            method='delete_many',
            model=models.Role,
            arguments={'where': where},
            root_selection=['count'],
        )



# NOTE: some arguments are meaningless in this context but are included
# for completeness sake
class UserBatchActions:
    def __init__(self, batcher: Batch) -> None:
        self._batcher = batcher

    def create(
        self,
        data: types.UserCreateInput,
        include: Optional[types.UserInclude] = None
    ) -> None:
        self._batcher._add(
            method='create',
            model=models.User,
            arguments={
                'data': data,
                'include': include,
            },
        )

    def create_many(
        self,
        data: List[types.UserCreateWithoutRelationsInput],
        *,
        skip_duplicates: Optional[bool] = None,
    ) -> None:
        if self._batcher._active_provider == 'sqlite':
            raise errors.UnsupportedDatabaseError('sqlite', 'create_many()')

        self._batcher._add(
            method='create_many',
            model=models.User,
            arguments={
                'data': data,
                'skipDuplicates': skip_duplicates,
            },
            root_selection=['count'],
        )

    def delete(
        self,
        where: types.UserWhereUniqueInput,
        include: Optional[types.UserInclude] = None,
    ) -> None:
        self._batcher._add(
            method='delete',
            model=models.User,
            arguments={
                'where': where,
                'include': include,
            },
        )

    def update(
        self,
        data: types.UserUpdateInput,
        where: types.UserWhereUniqueInput,
        include: Optional[types.UserInclude] = None
    ) -> None:
        self._batcher._add(
            method='update',
            model=models.User,
            arguments={
                'data': data,
                'where': where,
                'include': include,
            },
        )

    def upsert(
        self,
        where: types.UserWhereUniqueInput,
        data: types.UserUpsertInput,
        include: Optional[types.UserInclude] = None,
    ) -> None:
        self._batcher._add(
            method='upsert',
            model=models.User,
            arguments={
                'where': where,
                'include': include,
                'create': data.get('create'),
                'update': data.get('update'),
            },
        )

    def update_many(
        self,
        data: types.UserUpdateManyMutationInput,
        where: types.UserWhereInput,
    ) -> None:
        self._batcher._add(
            method='update_many',
            model=models.User,
            arguments={'data': data, 'where': where,},
            root_selection=['count'],
        )

    def delete_many(
        self,
        where: Optional[types.UserWhereInput] = None,
    ) -> None:
        self._batcher._add(
            method='delete_many',
            model=models.User,
            arguments={'where': where},
            root_selection=['count'],
        )



# NOTE: some arguments are meaningless in this context but are included
# for completeness sake
class DeviceFingerprintBatchActions:
    def __init__(self, batcher: Batch) -> None:
        self._batcher = batcher

    def create(
        self,
        data: types.DeviceFingerprintCreateInput,
        include: Optional[types.DeviceFingerprintInclude] = None
    ) -> None:
        self._batcher._add(
            method='create',
            model=models.DeviceFingerprint,
            arguments={
                'data': data,
                'include': include,
            },
        )

    def create_many(
        self,
        data: List[types.DeviceFingerprintCreateWithoutRelationsInput],
        *,
        skip_duplicates: Optional[bool] = None,
    ) -> None:
        if self._batcher._active_provider == 'sqlite':
            raise errors.UnsupportedDatabaseError('sqlite', 'create_many()')

        self._batcher._add(
            method='create_many',
            model=models.DeviceFingerprint,
            arguments={
                'data': data,
                'skipDuplicates': skip_duplicates,
            },
            root_selection=['count'],
        )

    def delete(
        self,
        where: types.DeviceFingerprintWhereUniqueInput,
        include: Optional[types.DeviceFingerprintInclude] = None,
    ) -> None:
        self._batcher._add(
            method='delete',
            model=models.DeviceFingerprint,
            arguments={
                'where': where,
                'include': include,
            },
        )

    def update(
        self,
        data: types.DeviceFingerprintUpdateInput,
        where: types.DeviceFingerprintWhereUniqueInput,
        include: Optional[types.DeviceFingerprintInclude] = None
    ) -> None:
        self._batcher._add(
            method='update',
            model=models.DeviceFingerprint,
            arguments={
                'data': data,
                'where': where,
                'include': include,
            },
        )

    def upsert(
        self,
        where: types.DeviceFingerprintWhereUniqueInput,
        data: types.DeviceFingerprintUpsertInput,
        include: Optional[types.DeviceFingerprintInclude] = None,
    ) -> None:
        self._batcher._add(
            method='upsert',
            model=models.DeviceFingerprint,
            arguments={
                'where': where,
                'include': include,
                'create': data.get('create'),
                'update': data.get('update'),
            },
        )

    def update_many(
        self,
        data: types.DeviceFingerprintUpdateManyMutationInput,
        where: types.DeviceFingerprintWhereInput,
    ) -> None:
        self._batcher._add(
            method='update_many',
            model=models.DeviceFingerprint,
            arguments={'data': data, 'where': where,},
            root_selection=['count'],
        )

    def delete_many(
        self,
        where: Optional[types.DeviceFingerprintWhereInput] = None,
    ) -> None:
        self._batcher._add(
            method='delete_many',
            model=models.DeviceFingerprint,
            arguments={'where': where},
            root_selection=['count'],
        )



# NOTE: some arguments are meaningless in this context but are included
# for completeness sake
class AuditLogBatchActions:
    def __init__(self, batcher: Batch) -> None:
        self._batcher = batcher

    def create(
        self,
        data: types.AuditLogCreateInput,
        include: Optional[types.AuditLogInclude] = None
    ) -> None:
        self._batcher._add(
            method='create',
            model=models.AuditLog,
            arguments={
                'data': data,
                'include': include,
            },
        )

    def create_many(
        self,
        data: List[types.AuditLogCreateWithoutRelationsInput],
        *,
        skip_duplicates: Optional[bool] = None,
    ) -> None:
        if self._batcher._active_provider == 'sqlite':
            raise errors.UnsupportedDatabaseError('sqlite', 'create_many()')

        self._batcher._add(
            method='create_many',
            model=models.AuditLog,
            arguments={
                'data': data,
                'skipDuplicates': skip_duplicates,
            },
            root_selection=['count'],
        )

    def delete(
        self,
        where: types.AuditLogWhereUniqueInput,
        include: Optional[types.AuditLogInclude] = None,
    ) -> None:
        self._batcher._add(
            method='delete',
            model=models.AuditLog,
            arguments={
                'where': where,
                'include': include,
            },
        )

    def update(
        self,
        data: types.AuditLogUpdateInput,
        where: types.AuditLogWhereUniqueInput,
        include: Optional[types.AuditLogInclude] = None
    ) -> None:
        self._batcher._add(
            method='update',
            model=models.AuditLog,
            arguments={
                'data': data,
                'where': where,
                'include': include,
            },
        )

    def upsert(
        self,
        where: types.AuditLogWhereUniqueInput,
        data: types.AuditLogUpsertInput,
        include: Optional[types.AuditLogInclude] = None,
    ) -> None:
        self._batcher._add(
            method='upsert',
            model=models.AuditLog,
            arguments={
                'where': where,
                'include': include,
                'create': data.get('create'),
                'update': data.get('update'),
            },
        )

    def update_many(
        self,
        data: types.AuditLogUpdateManyMutationInput,
        where: types.AuditLogWhereInput,
    ) -> None:
        self._batcher._add(
            method='update_many',
            model=models.AuditLog,
            arguments={'data': data, 'where': where,},
            root_selection=['count'],
        )

    def delete_many(
        self,
        where: Optional[types.AuditLogWhereInput] = None,
    ) -> None:
        self._batcher._add(
            method='delete_many',
            model=models.AuditLog,
            arguments={'where': where},
            root_selection=['count'],
        )



# NOTE: some arguments are meaningless in this context but are included
# for completeness sake
class EmployeeProfileBatchActions:
    def __init__(self, batcher: Batch) -> None:
        self._batcher = batcher

    def create(
        self,
        data: types.EmployeeProfileCreateInput,
        include: Optional[types.EmployeeProfileInclude] = None
    ) -> None:
        self._batcher._add(
            method='create',
            model=models.EmployeeProfile,
            arguments={
                'data': data,
                'include': include,
            },
        )

    def create_many(
        self,
        data: List[types.EmployeeProfileCreateWithoutRelationsInput],
        *,
        skip_duplicates: Optional[bool] = None,
    ) -> None:
        if self._batcher._active_provider == 'sqlite':
            raise errors.UnsupportedDatabaseError('sqlite', 'create_many()')

        self._batcher._add(
            method='create_many',
            model=models.EmployeeProfile,
            arguments={
                'data': data,
                'skipDuplicates': skip_duplicates,
            },
            root_selection=['count'],
        )

    def delete(
        self,
        where: types.EmployeeProfileWhereUniqueInput,
        include: Optional[types.EmployeeProfileInclude] = None,
    ) -> None:
        self._batcher._add(
            method='delete',
            model=models.EmployeeProfile,
            arguments={
                'where': where,
                'include': include,
            },
        )

    def update(
        self,
        data: types.EmployeeProfileUpdateInput,
        where: types.EmployeeProfileWhereUniqueInput,
        include: Optional[types.EmployeeProfileInclude] = None
    ) -> None:
        self._batcher._add(
            method='update',
            model=models.EmployeeProfile,
            arguments={
                'data': data,
                'where': where,
                'include': include,
            },
        )

    def upsert(
        self,
        where: types.EmployeeProfileWhereUniqueInput,
        data: types.EmployeeProfileUpsertInput,
        include: Optional[types.EmployeeProfileInclude] = None,
    ) -> None:
        self._batcher._add(
            method='upsert',
            model=models.EmployeeProfile,
            arguments={
                'where': where,
                'include': include,
                'create': data.get('create'),
                'update': data.get('update'),
            },
        )

    def update_many(
        self,
        data: types.EmployeeProfileUpdateManyMutationInput,
        where: types.EmployeeProfileWhereInput,
    ) -> None:
        self._batcher._add(
            method='update_many',
            model=models.EmployeeProfile,
            arguments={'data': data, 'where': where,},
            root_selection=['count'],
        )

    def delete_many(
        self,
        where: Optional[types.EmployeeProfileWhereInput] = None,
    ) -> None:
        self._batcher._add(
            method='delete_many',
            model=models.EmployeeProfile,
            arguments={'where': where},
            root_selection=['count'],
        )



# NOTE: some arguments are meaningless in this context but are included
# for completeness sake
class AttendanceRecordBatchActions:
    def __init__(self, batcher: Batch) -> None:
        self._batcher = batcher

    def create(
        self,
        data: types.AttendanceRecordCreateInput,
        include: Optional[types.AttendanceRecordInclude] = None
    ) -> None:
        self._batcher._add(
            method='create',
            model=models.AttendanceRecord,
            arguments={
                'data': data,
                'include': include,
            },
        )

    def create_many(
        self,
        data: List[types.AttendanceRecordCreateWithoutRelationsInput],
        *,
        skip_duplicates: Optional[bool] = None,
    ) -> None:
        if self._batcher._active_provider == 'sqlite':
            raise errors.UnsupportedDatabaseError('sqlite', 'create_many()')

        self._batcher._add(
            method='create_many',
            model=models.AttendanceRecord,
            arguments={
                'data': data,
                'skipDuplicates': skip_duplicates,
            },
            root_selection=['count'],
        )

    def delete(
        self,
        where: types.AttendanceRecordWhereUniqueInput,
        include: Optional[types.AttendanceRecordInclude] = None,
    ) -> None:
        self._batcher._add(
            method='delete',
            model=models.AttendanceRecord,
            arguments={
                'where': where,
                'include': include,
            },
        )

    def update(
        self,
        data: types.AttendanceRecordUpdateInput,
        where: types.AttendanceRecordWhereUniqueInput,
        include: Optional[types.AttendanceRecordInclude] = None
    ) -> None:
        self._batcher._add(
            method='update',
            model=models.AttendanceRecord,
            arguments={
                'data': data,
                'where': where,
                'include': include,
            },
        )

    def upsert(
        self,
        where: types.AttendanceRecordWhereUniqueInput,
        data: types.AttendanceRecordUpsertInput,
        include: Optional[types.AttendanceRecordInclude] = None,
    ) -> None:
        self._batcher._add(
            method='upsert',
            model=models.AttendanceRecord,
            arguments={
                'where': where,
                'include': include,
                'create': data.get('create'),
                'update': data.get('update'),
            },
        )

    def update_many(
        self,
        data: types.AttendanceRecordUpdateManyMutationInput,
        where: types.AttendanceRecordWhereInput,
    ) -> None:
        self._batcher._add(
            method='update_many',
            model=models.AttendanceRecord,
            arguments={'data': data, 'where': where,},
            root_selection=['count'],
        )

    def delete_many(
        self,
        where: Optional[types.AttendanceRecordWhereInput] = None,
    ) -> None:
        self._batcher._add(
            method='delete_many',
            model=models.AttendanceRecord,
            arguments={'where': where},
            root_selection=['count'],
        )



# NOTE: some arguments are meaningless in this context but are included
# for completeness sake
class PayrollRecordBatchActions:
    def __init__(self, batcher: Batch) -> None:
        self._batcher = batcher

    def create(
        self,
        data: types.PayrollRecordCreateInput,
        include: Optional[types.PayrollRecordInclude] = None
    ) -> None:
        self._batcher._add(
            method='create',
            model=models.PayrollRecord,
            arguments={
                'data': data,
                'include': include,
            },
        )

    def create_many(
        self,
        data: List[types.PayrollRecordCreateWithoutRelationsInput],
        *,
        skip_duplicates: Optional[bool] = None,
    ) -> None:
        if self._batcher._active_provider == 'sqlite':
            raise errors.UnsupportedDatabaseError('sqlite', 'create_many()')

        self._batcher._add(
            method='create_many',
            model=models.PayrollRecord,
            arguments={
                'data': data,
                'skipDuplicates': skip_duplicates,
            },
            root_selection=['count'],
        )

    def delete(
        self,
        where: types.PayrollRecordWhereUniqueInput,
        include: Optional[types.PayrollRecordInclude] = None,
    ) -> None:
        self._batcher._add(
            method='delete',
            model=models.PayrollRecord,
            arguments={
                'where': where,
                'include': include,
            },
        )

    def update(
        self,
        data: types.PayrollRecordUpdateInput,
        where: types.PayrollRecordWhereUniqueInput,
        include: Optional[types.PayrollRecordInclude] = None
    ) -> None:
        self._batcher._add(
            method='update',
            model=models.PayrollRecord,
            arguments={
                'data': data,
                'where': where,
                'include': include,
            },
        )

    def upsert(
        self,
        where: types.PayrollRecordWhereUniqueInput,
        data: types.PayrollRecordUpsertInput,
        include: Optional[types.PayrollRecordInclude] = None,
    ) -> None:
        self._batcher._add(
            method='upsert',
            model=models.PayrollRecord,
            arguments={
                'where': where,
                'include': include,
                'create': data.get('create'),
                'update': data.get('update'),
            },
        )

    def update_many(
        self,
        data: types.PayrollRecordUpdateManyMutationInput,
        where: types.PayrollRecordWhereInput,
    ) -> None:
        self._batcher._add(
            method='update_many',
            model=models.PayrollRecord,
            arguments={'data': data, 'where': where,},
            root_selection=['count'],
        )

    def delete_many(
        self,
        where: Optional[types.PayrollRecordWhereInput] = None,
    ) -> None:
        self._batcher._add(
            method='delete_many',
            model=models.PayrollRecord,
            arguments={'where': where},
            root_selection=['count'],
        )



# NOTE: some arguments are meaningless in this context but are included
# for completeness sake
class LeaveRequestBatchActions:
    def __init__(self, batcher: Batch) -> None:
        self._batcher = batcher

    def create(
        self,
        data: types.LeaveRequestCreateInput,
        include: Optional[types.LeaveRequestInclude] = None
    ) -> None:
        self._batcher._add(
            method='create',
            model=models.LeaveRequest,
            arguments={
                'data': data,
                'include': include,
            },
        )

    def create_many(
        self,
        data: List[types.LeaveRequestCreateWithoutRelationsInput],
        *,
        skip_duplicates: Optional[bool] = None,
    ) -> None:
        if self._batcher._active_provider == 'sqlite':
            raise errors.UnsupportedDatabaseError('sqlite', 'create_many()')

        self._batcher._add(
            method='create_many',
            model=models.LeaveRequest,
            arguments={
                'data': data,
                'skipDuplicates': skip_duplicates,
            },
            root_selection=['count'],
        )

    def delete(
        self,
        where: types.LeaveRequestWhereUniqueInput,
        include: Optional[types.LeaveRequestInclude] = None,
    ) -> None:
        self._batcher._add(
            method='delete',
            model=models.LeaveRequest,
            arguments={
                'where': where,
                'include': include,
            },
        )

    def update(
        self,
        data: types.LeaveRequestUpdateInput,
        where: types.LeaveRequestWhereUniqueInput,
        include: Optional[types.LeaveRequestInclude] = None
    ) -> None:
        self._batcher._add(
            method='update',
            model=models.LeaveRequest,
            arguments={
                'data': data,
                'where': where,
                'include': include,
            },
        )

    def upsert(
        self,
        where: types.LeaveRequestWhereUniqueInput,
        data: types.LeaveRequestUpsertInput,
        include: Optional[types.LeaveRequestInclude] = None,
    ) -> None:
        self._batcher._add(
            method='upsert',
            model=models.LeaveRequest,
            arguments={
                'where': where,
                'include': include,
                'create': data.get('create'),
                'update': data.get('update'),
            },
        )

    def update_many(
        self,
        data: types.LeaveRequestUpdateManyMutationInput,
        where: types.LeaveRequestWhereInput,
    ) -> None:
        self._batcher._add(
            method='update_many',
            model=models.LeaveRequest,
            arguments={'data': data, 'where': where,},
            root_selection=['count'],
        )

    def delete_many(
        self,
        where: Optional[types.LeaveRequestWhereInput] = None,
    ) -> None:
        self._batcher._add(
            method='delete_many',
            model=models.LeaveRequest,
            arguments={'where': where},
            root_selection=['count'],
        )



# NOTE: some arguments are meaningless in this context but are included
# for completeness sake
class ShiftBatchActions:
    def __init__(self, batcher: Batch) -> None:
        self._batcher = batcher

    def create(
        self,
        data: types.ShiftCreateInput,
        include: Optional[types.ShiftInclude] = None
    ) -> None:
        self._batcher._add(
            method='create',
            model=models.Shift,
            arguments={
                'data': data,
                'include': include,
            },
        )

    def create_many(
        self,
        data: List[types.ShiftCreateWithoutRelationsInput],
        *,
        skip_duplicates: Optional[bool] = None,
    ) -> None:
        if self._batcher._active_provider == 'sqlite':
            raise errors.UnsupportedDatabaseError('sqlite', 'create_many()')

        self._batcher._add(
            method='create_many',
            model=models.Shift,
            arguments={
                'data': data,
                'skipDuplicates': skip_duplicates,
            },
            root_selection=['count'],
        )

    def delete(
        self,
        where: types.ShiftWhereUniqueInput,
        include: Optional[types.ShiftInclude] = None,
    ) -> None:
        self._batcher._add(
            method='delete',
            model=models.Shift,
            arguments={
                'where': where,
                'include': include,
            },
        )

    def update(
        self,
        data: types.ShiftUpdateInput,
        where: types.ShiftWhereUniqueInput,
        include: Optional[types.ShiftInclude] = None
    ) -> None:
        self._batcher._add(
            method='update',
            model=models.Shift,
            arguments={
                'data': data,
                'where': where,
                'include': include,
            },
        )

    def upsert(
        self,
        where: types.ShiftWhereUniqueInput,
        data: types.ShiftUpsertInput,
        include: Optional[types.ShiftInclude] = None,
    ) -> None:
        self._batcher._add(
            method='upsert',
            model=models.Shift,
            arguments={
                'where': where,
                'include': include,
                'create': data.get('create'),
                'update': data.get('update'),
            },
        )

    def update_many(
        self,
        data: types.ShiftUpdateManyMutationInput,
        where: types.ShiftWhereInput,
    ) -> None:
        self._batcher._add(
            method='update_many',
            model=models.Shift,
            arguments={'data': data, 'where': where,},
            root_selection=['count'],
        )

    def delete_many(
        self,
        where: Optional[types.ShiftWhereInput] = None,
    ) -> None:
        self._batcher._add(
            method='delete_many',
            model=models.Shift,
            arguments={'where': where},
            root_selection=['count'],
        )



Client = Prisma